...
- Cluster admission policy is deferred for now; do not add a second policy layer until the data-access model needs it.
...
- Keycloak namespace: `frigg-pot-platform` for the in-cluster Keycloak Helm release.
...
- Keycloak users are the login identities.
- Polaris principals are the authorization identities inside Polaris.
- Use the same email address for the human user identity where possible so operators can correlate records easily.
- Create Keycloak realm roles with a `POLARIS_` prefix, then map those roles into Polaris principal roles.
- Keep Keycloak groups for administration convenience if you want, but ensure the effective roles exposed in the token still use the `POLARIS_` prefix.
- Keycloak realms and Polaris realms are separate concepts and do not need a 1:1 match; use consistent naming instead of assuming shared objects.
...
POLARIS_OIDC_AUTH_SERVER_URL=http://keycloak.frigg-pot-platform.svc.cluster.local/realms/<realm>
...
trino:
	oidc:
		enabled: true
			issuer: "http://keycloak.frigg-pot-platform.svc.cluster.local/realms/frigg-data-platform"
		clientId: "trino"
		createSecret: true
...
```bash
TRINO_OIDC_ENABLED=true
TRINO_OIDC_ISSUER=http://keycloak.frigg-pot-platform.svc.cluster.local/realms/frigg-data-platform
TRINO_OIDC_CLIENT_ID=trino
...
1. Port-forward the Keycloak service:
	- `kubectl -n frigg-pot-platform port-forward svc/keycloak 8081:80`
...
- For this cluster, working in-cluster value is `http://keycloak.frigg-pot-platform.svc.cluster.local/realms/frigg-data-platform`.
- `QUARKUS_OIDC_TLS_VERIFICATION` can be empty (default verification) or `none` if cert trust is not configured yet.

### 1. Check pods are running
```bash
kubectl -n frigg-pot-platform get po,svc | rg 'keycloak|postgresql'
...
kubectl -n frigg-pot-platform get secret keycloak -o jsonpath='{.data.admin-password}' | base64 -d
echo
...
```bash
KEYCLOAK_POD=$(kubectl -n frigg-pot-platform get pod -l app.kubernetes.io/name=keycloak -o jsonpath='{.items[0].metadata.name}')

kubectl -n frigg-pot-platform exec "$KEYCLOAK_POD" -- env HOME=/tmp KCADM_CONFIG=/tmp/kcadm.config \
	/opt/bitnami/keycloak/bin/kcadm.sh config credentials \
	--server http://127.0.0.1:8080 --realm master --user admin --password "$(kubectl -n frigg-pot-platform get secret keycloak -o jsonpath='{.data.admin-password}' | base64 -d)"
